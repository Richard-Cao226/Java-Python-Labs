...
{ 
    ...
    public ArrayList<PhoneEntry> search( PhoneEntry pEntry )
    {
        ...
        if(!pEntry.getFirstName().equals("") && !pEntry.getLastName().equals("")) {
        	int index = phoneBook.indexOf(pEntry);
        	if(index > -1) {
        		result.add(pEntry);
        	}
        } else if(pEntry.getFirstName().equals("") && !pEntry.getLastName().equals("")) {
    		for(int i = 0; i < phoneBook.size(); i++) {
    			if(phoneBook.get(i).getLastName().equalsIgnoreCase(pEntry.getLastName())) {
    				result.add(pEntry);
    			}
    		}
        }
