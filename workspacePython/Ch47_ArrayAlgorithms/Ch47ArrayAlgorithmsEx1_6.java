...
public class Ch47ArrayAlgorithmsEx1_6
{
    ...
    {
    	int temp;
        for ( int j = 0, end = data.length-1; j < data.length/2; j++, end-- )
        {
            temp = data[j];
            data[j] = data[end];
            data[end] = temp;
    ...
    public int[] Ex4_ReverserVersion2( int[] data )
    {
        ...
        for ( int j = 0, i = data.length-1; j < data.length; j++, i-- )
        {
            result[j] = data[i];
        }
    ...
    public double[] Ex6_DataTweeker( String fileName ) throws IOException
    {
        ...
        mostDistant = array[0];
        for(int i = 0; i < array.length; i++) {
        	if(Math.abs(array[i]-average)>Math.abs(mostDistant-average)) {
        		mostDistant = array[i];
        	}
        }
        int count = 0;
        for(int i = 0; i < array.length; i++) {
        	if(array[i] != mostDistant) {
        		newAverage += array[i];
        		count++;
        	}
        }
