...
def BlackjackGame():
    ...
    while(1):
        try:
            ...
            playerMoney = input('Please enter an integer. ')
    ...
    while(1):
        try:
            ...
            rounds = input('Please enter an integer. ')
    print()
    ...
    for round in range(1,rounds+1):
        ...
        while(1):
            try:
                ...
                playerBet = input('The betting limits for each round are $2 to $500. Please enter an integer from 2 to 500. ')
                continue
            try:
        ...
        if player.hand.getTotalWithAce() == 21 and dealer.hand.getTotalWithAce() < 21:
            ...
            if player.hand.getTotalWithAce() == 9 or player.hand.getTotalWithAce() == 10 or player.hand.getTotalWithAce() == 11:
                double = input('Your hand totals the value of {}. Would you like to double down? (y/n)'.format(player.hand.getTotalWithAce()))
                while(1):
                    if double == 'y':
                        playerBet += playerBet
                        player.addCard(deck.deal())
